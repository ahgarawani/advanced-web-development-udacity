...
def create_app(test_config=None):
  ...
  def play_quiz():
    ...
    try:
      remaining_questions = Question.query.filter(Question.category == quiz_category['id'] if quiz_category['id'] != 0 else not quiz_category['id']).filter(Question.id.notin_((previous_questions))).all()
      new_question = Question.query.get(random.choice(remaining_questions).id).format() if len(remaining_questions) > 0 else None
      return jsonify({
        'success': True,
